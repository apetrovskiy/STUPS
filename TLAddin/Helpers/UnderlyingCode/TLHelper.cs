...
//        }
        ...
        internal static Meyn.TestLink.TestProject[] GetProjectsByName(TLSCmdletBase cmdlet, string[] projectNames)
        {
            ...
            try {
                ...
                cmdlet.WriteVerbose(cmdlet, "iterating through the project names colection");
...
//        }
        ...
        public static void AddBuild(
            ...
            string[] buildNotes)
        {
            string buildNameNow = string.Empty;
            
            try {
                for (int j = 0; j < testPlans.Length; j++) {
                    for (int i = 0; i < buildNames.Length; i++) {
                        
                        buildNameNow = buildNames[i];
                    
                        string description = string.Empty;
                        if (null != buildNotes[i] && string.Empty != buildNotes[i]) {
                            description = buildNotes[i];
                        }
...
//                        cmdlet.WriteObject(
//                            cmdlet,
//                            TLAddinData.CurrentTestLinkConnection.GetLatestBuildForTestPlan(testPlan.id));
//                    }
//                
//                }
//                
