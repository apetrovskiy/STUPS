...
# Source code
Start-Process -FilePath "xcopy" -ArgumentList @("$($pathToProjectRoot)\UIA\*.cs*", "$($pathToProjectRoot)\out\$($DirName)\sources\UIA\", "/s", "/v", "/i") -NoNewWindow;
Start-Process -FilePath "xcopy" -ArgumentList @("$($pathToProjectRoot)\PS\*.cs*", "$($pathToProjectRoot)\out\$($DirName)\sources\PS\", "/s", "/v", "/i") -NoNewWindow;
Start-Process -FilePath "xcopy" -ArgumentList @("$($pathToProjectRoot)\PSTestLib\*.cs*", "$($pathToProjectRoot)\out\$($DirName)\sources\PSTestLib\", "/s", "/v", "/i") -NoNewWindow;
Copy-Item -Path "$($pathToProjectRoot)\*.sln" -Destination "$($pathToProjectRoot)\out\$($DirName)\sources";

# Samples
Start-Process -FilePath "xcopy" -ArgumentList @("$($pathToProjectRoot)\samples\UIAutomation\*.ps*", "$($pathToProjectRoot)\out\$($DirName)\samples", "/s", "/v", "/i") -NoNewWindow;